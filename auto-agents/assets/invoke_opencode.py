"""Subprocess wrapper for the `opencode` (sst/opencode) CLI.

Known-good invocation pattern: `opencode run "<prompt>"`. Override with
AUTO_AGENTS_OPENCODE_CMD if your install differs (the env value is shell-split
...
def _resolved_cmd() -> list[str]:
    raw = os.environ.get("AUTO_AGENTS_OPENCODE_CMD", DEFAULT_CMD)
...
def extract_answer(stdout: str) -> str:
    """`opencode run` prints assistant text directly. Trim trailing blanks."""
